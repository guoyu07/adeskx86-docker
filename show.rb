...
def func2(file)
	...
	if ok_num == 0 
		...
		puts "* 执行结果为成功的容器中，最找开始执行的容器是 #{start_times[0][1]}，最晚开始执行的容器是 #{start_times[-1][1]}。"
		#puts "* Success Client End from #{end_times[0][1]['EndTime']} to #{end_times[-1][1]['EndTime']}"
		puts "* 执行结果为成功的容器中，最早执行完成的容器是 #{end_times[0][1]}，最晚执行完成的容器是 #{end_times[-1][1]}"
		#puts "* Success Query Time slowest is #{user_times[-1]}"
		puts "* 执行结果为成功的容器中，执行速度最快的容器是 #{user_times[-1]}。"
